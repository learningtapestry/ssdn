...
export default function AssociateQueue(props: AssociateQueueProps) {
  ...
  const renderExternalQueue = () => (
    <Fragment>
      ...
      <p className="mt-3">
        <small>
          Before associating an external SQS queue, please make sure your AWS account has the
          required access permissions. More specifically, the <em>ReceiveMessage</em>,{" "}
          <em>DeleteMessage</em> and <em>GetQueueAttributes</em> permissions are needed. Check out
          the official documentation on how to add a{" "}
      ...
      <div>
        <Form.Group controlId="externalAccount">
          <Form.Check
            ...
            onChange={handleChangeExternalQueue}
          />
        </Form.Group>
        <Form.Group controlId="queueArn">
          {externalQueue ? renderExternalQueue() : renderOwnQueue()}
