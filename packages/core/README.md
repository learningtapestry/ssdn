...
This is the **core** component in the SSDN project. It handles all the basic features that a regular SSDN instance
provides.
...
After that, you'll need [Node.js](https://nodejs.org/en/download/) version 10.16 or higher, as well
...
for your API. For example, to execute the `ProcessXAPIStatement` lambda function using a sample event that's included in
the project, run:
...
This will allow you to access `http://localhost:3000/xAPI/statements` and call the lambda code from an HTTP
endpoint.
...
`SSDN_LOG_LEVEL`, which accepts all the levels that Pino does. In tests, the log level should be set to
